...
with conn.cursor() as cursor:
    sql = 'select distinct author_ID, company ' \
          ...
          'and is_core = %s'
    cursor.execute(sql, ('unknown', 'bot', 1))
    res = cursor.fetchall()
    print(res)
conn.commit()
...
with conn.cursor() as cursor:
    sql = 'select id, author_ID, sum_lines ' \
          ...
          'and is_core = %s ' \
          'order by author_ID'
    cursor.execute(sql, ('volunteer', 1))
    res = cursor.fetchall()
    # print(res)
conn.commit()
...
with conn.cursor() as cursor:
    sql = 'select id, author_ID, sum_lines ' \
          ...
          'and is_core = %s ' \
          'order by author_ID'
    cursor.execute(sql, ('volunteer', 'unknown', 'bot', 1))
    res = cursor.fetchall()
conn.commit()
...
if pVal < 0.05:
    print("have statitical difference", pVal)
else:
    print("no statitical difference", pVal)
...
with conn.cursor() as cursor:
    sql = 'select author_ID, company, min(author_date), max(author_date), count(*) ' \
          'from icse24 ' \
          'where company != %s and company != %s and is_core = %s ' \
...
print('median_vltr frequency', median(vltr_freq))
print('median_com frequency', median(com_freq))
# both p-value < 0.05, indicating the two distributions are not norm. Thus we choose Mann-Whitney U
...
# np.savetxt(path + "/data/icse24/id_frequency.csv", id_interval, fmt="%s", delimiter=",")
...
role_type_freq = pd.read_csv(path + "/data/icse24/role_type_freq.csv")
...
plt.savefig(path + "/pic/extension/core_task.pdf", format='pdf')
plt.show()
...
def compare_difference(data1, data2, feature):
    ...
    if p_value1 < 0.05 or p_value2 < 0.05:
        ...
        if pVal < 0.005:
            print(feature, ": have statitical difference", pVal)
        else:
            print(feature, ": no statitical difference", pVal)
